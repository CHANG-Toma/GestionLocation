...
public class demandes_DAO {
    ...
    public void addDemande(int tailleAppart, int prix_min, int prix_max, String villeAppart, int numClient, String date_limite, int numType, int numArrondi, String departement) throws SQLException {
        connexionBDD cnt = new connexionBDD(); // Connexion à la BDD

        String sql = "INSERT INTO demandes (num_cli, date_limite, prix_min, prix_max, taille, ville, idType, idArrondi, departement) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void deleteVisite(int num_cli) throws SQLException {
        connexionBDD cnt = new connexionBDD(); // Connexion à la BDD
        PreparedStatement psReq = null;
        try {
            // Requete SQL à executer avec un paramètre
            String req = "DELETE FROM visiter where idClient = ?";
            psReq = cnt.prepareReq(req); // Utilisation de prepareStatement() au lieu de prepareReq()
            psReq.setInt(1, num_cli);
            // Execution de la requete
            psReq.executeUpdate(); // Utilisation de executeUpdate() au lieu de executeQuery()
